...
| `DISCORD_BOT_SECRET` | **Recommended.** Same random string in Railway **and** Supabase (`pnpm supabase secrets set DISCORD_BOT_SECRET=...`). Bot sends `x-bot-secret` to Edge Functions — avoids **401 Unauthorized** when Bearer service_role does not match what Edge sees. |
