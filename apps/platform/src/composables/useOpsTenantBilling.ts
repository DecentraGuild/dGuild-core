...
export function useOpsTenantBilling(
  ...
  const watchtowerTracks = computed(() => {
    ...
    if (limits.length) {
      return limits.map((r) => ({ scopeKey: r.meter_key, label: watchtowerScopeLabel(r.meter_key), count: r.quantity_total }))
    }
    ...
    if (rows.length) {
      return rows.map((r) => {
        ...
        return { scopeKey: key, label: watchtowerScopeLabel(key), count: Number(cond[key]) || 0 }
      })
    }
    return WATCHTOWER_METER_KEYS.map((key) => ({ scopeKey: key, label: watchtowerScopeLabel(key), count: 0 }))
  })
  ...
  watch(
    ...
    (tracks) => {
      ...
      for (const t of tracks) next[t.scopeKey] = t.count
