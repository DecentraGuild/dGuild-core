<template>
  <div class="mint-picker">
    <div class="mint-picker__add">
      <div class="mint-picker__add-row">
        <TextInput
          v-model="newMint"
          :placeholder="inputPlaceholder"
          :error="addError"
          class="mint-picker__input"
        ...
        <select
          ...
          class="mint-picker__select"
        >
          <option value="auto">Auto-detect type</option>
          <option value="SPL">SPL token</option>
          <option value="NFT">NFT / collection</option>
        </select>
        <Button variant="secondary" :disabled="!canAdd || adding" @click="addMint">
          <Icon v-if="adding" icon="mdi:loading" class="mint-picker__spinner" />
          {{ adding ? 'Loading…' : 'Load' }}
        </Button>
      </div>
      <p v-if="hint" class="mint-picker__hint">{{ hint }}</p>
    </div>
        ...
        >
          ...
          >
            <div class="mint-picker__thumb">
              ...
              <span v-else class="mint-picker__thumb-placeholder"><Icon icon="mdi:image-off" /></span>
            </div>
            <div class="mint-picker__detail">
              <template v-if="item._loading">
                <Icon icon="mdi:loading" class="mint-picker__spinner" />
                <span class="mint-picker__name">{{ truncate(item.mint) }}</span>
              ...
              <template v-else>
                <span class="mint-picker__name">{{ item.name || truncate(item.mint) }}</span>
                <code class="mint-picker__address">{{ item.mint.slice(0, 8) }}...{{ item.mint.slice(-6) }}</code>
          ...
          <div class="mint-picker__actions">
            ...
            <div v-if="showTrackingTiers" class="mint-picker__tier">
              <span class="mint-picker__tier-always">
                <Icon icon="mdi:check" class="mint-picker__tier-icon" />
                <span>Metadata</span>
              </span>
        ...
        >
          ...
          >
            <div class="mint-picker__thumb">
              ...
              <span v-else class="mint-picker__thumb-placeholder"><Icon icon="mdi:token" /></span>
            </div>
            <div class="mint-picker__detail">
              <template v-if="item._loading">
                <Icon icon="mdi:loading" class="mint-picker__spinner" />
              ...
              <template v-else>
                <span class="mint-picker__name">{{ (item.symbol || item.name) || truncate(item.mint) }}</span>
                <code class="mint-picker__address">{{ item.mint.slice(0, 8) }}...{{ item.mint.slice(-6) }}</code>
                <span class="mint-picker__meta">
...
import { useMintResolution } from '~/composables/useMintResolution'
...
function onAddressBookSelect(mint: string, entry: AddressBookEntry) {
  newMint.value = mint
  if (entry.kind) selectedKind.value = entry.kind
  emit('address-book-select', mint, entry)
}

const canAdd = computed(() => {
  const trimmed = newMint.value.trim()
  return trimmed.length >= 32 && !adding.value
})
...
async function addMint() {
  ...
  try {
    const result = await resolveMint(mint, selectedKind.value === 'auto' ? undefined : selectedKind.value)
