      ...
      <div class="escrow-modal__body">
        <section v-if="isMaker" class="escrow-modal__owner">
          ...
          <EscrowDetailActions
            ...
            @cancel="() => detail.handleCancel(afterTerminate)"
          />
          ...
          <EscrowDetailMetadata
            ...
            :wallet-label="detail.walletLabel"
        ...
        <section v-if="!isMaker" class="escrow-modal__fill">
          ...
          <EscrowDetailTradeFlow
            ...
            @focus-amount-input="detail.onFocusAmountInput"
            @toggle-ratio="detail.toggleRatio"
            @fill="detail.handleFill(afterTerminate)"
...
const modelValueRef = toRef(props, 'active')
...
watch(
  () =>
    [
      props.variant,
      props.active,
      unref(detail.depositNameDisplay),
      unref(detail.requestNameDisplay),
      props.escrowId,
    ] as const,
  ([variant, active, dep, req, id]) => {
    if (variant !== 'page' || !active) return
    const label = dep || req || (id && id.length > 8 ? `${id.slice(0, 4)}…${id.slice(-4)}` : id || '')
    useHead({ title: label ? `Trade · ${label}` : 'Trade' })
  },
  { immediate: true }
)
</script>
